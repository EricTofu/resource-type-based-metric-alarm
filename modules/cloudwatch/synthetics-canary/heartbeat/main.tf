...
resource "aws_cloudwatch_metric_alarm" "success_percent" {
  ...
  alarm_description = "[${coalesce(try(each.value.overrides.severity, null), local.default_severities.success_percent)}]-${coalesce(
    try(each.value.overrides.description, null),
    "${var.project}-Synthetics-[${each.value.name}]-SuccessPercent dropped below threshold"
  )}"
  ...
  threshold = coalesce(
    try(each.value.overrides.success_percent_threshold, null),
    var.default_success_percent_threshold
  )
  ...
  alarm_actions = each.value.enabled ? [
    var.sns_topic_arns[coalesce(
      ...
      local.default_severities.success_percent
    )]
  ] : []
