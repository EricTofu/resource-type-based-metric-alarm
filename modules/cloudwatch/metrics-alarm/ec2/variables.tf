...
variable "resources" {
  ...
  validation {
    condition = alltrue([
      for r in var.resources :
      try(r.overrides.memory_threshold, null) == null || try(r.overrides.memory_threshold, 0) >= 0
    ])
    error_message = "overrides.memory_threshold must be non-negative or omitted."
  }
}
