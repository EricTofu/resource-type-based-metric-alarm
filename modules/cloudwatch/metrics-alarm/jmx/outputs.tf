output "alarm_arns" {
  ...
  value       = { for k, v in aws_cloudwatch_metric_alarm.heap_used : "${k}:HeapUsedBytes" => v.arn }
}

output "alarm_names" {
  description = "Map of <resource-key>:<metric-name> to alarm name for every alarm this module creates."
  value       = { for k, v in aws_cloudwatch_metric_alarm.heap_used : "${k}:HeapUsedBytes" => v.alarm_name }
}
