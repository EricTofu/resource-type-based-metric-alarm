...
`scripts/check_ec2_mem_metric.sh`, `scripts/check_asg_metrics.sh`, `scripts/check_asg_fleet_metrics.sh`, `scripts/check_s3_metrics.sh`, and `scripts/check_jmx_metrics.sh` verify that prerequisite CloudWatch metrics exist before alarms are applied (the JMX check matters most: both JMX alarms treat missing data as notBreaching, so a host without the cwagent JMX config would otherwise sit green forever). The ASG fleet check runs the alarms' actual Metrics Insights SELECTs via `get-metric-data` — the capacity and CPU queries prove tag telemetry on `AWS/AutoScaling` and `AWS/EC2` (separate paths), the CWAgent queries prove the `AppName` series end-to-end, and the heap queries are `ProcessGroupName`-scoped when the entry sets `process_group`, matching the alarm — and sanity-checks `heap_max_bytes` against observed `jvm.memory.heap.max` (±10%); it needs `cloudwatch:GetMetricData` on the preflight role. They accept `--tfvars <path>` and are invoked by `.github/workflows/preflight.yml` on PRs that touch `stacks/projects/**/terraform.tfvars`. Requires `PREFLIGHT_READ_ROLE_ARN` GitHub secret (read-only CloudWatch/EC2/S3 IAM role).
