...
Every fleet alarm — ASG `cpu`/`memory`/`disk`, JMX `heap_used` — is one alarm watching many instances via `GROUP BY InstanceId`. Each returned series is a **contributor**; the alarm enters ALARM as soon as one contributor breaches, and membership re-resolves at every evaluation (that is what makes CodeDeploy churn free).
...
2. **Metric math cannot wrap a multi-series query.** The exception above is only for a *Metrics Insights* expression. `DIFF(q1)` / `RATE(q1)` over a `GROUP BY` query is metric math and is rejected no matter what `q1` contains. This is what killed JMX `gc_time`; a math expression may back an alarm only when it resolves to a single series (EFS `throughput_util` does).
